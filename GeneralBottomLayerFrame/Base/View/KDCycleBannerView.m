...
@property (strong, nonatomic) CompleteBlockA completeBlock;
...
- (void)loadData {
    ...
    _pageControl.frame = CGRectMake(0, 0, 10 * _datasourceImages.count, 30);
    _pageControl.center = CGPointMake(CGRectGetWidth(_scrollView.frame) - 10 * _datasourceImages.count, CGRectGetHeight(_scrollView.frame) - 12);
...
//            [imageView addObserver:self forKeyPath:@"image" options:NSKeyValueObservingOptionNew|NSKeyValueObservingOptionOld context:kContentImageViewObservationContext];
            [imageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",imageSource]]];
...
- (void)reloadDataWithCompleteBlock:(CompleteBlockA)competeBlock {
...
- (void)singleTapGestureRecognizer:(UITapGestureRecognizer *)tapGesture {
    CGFloat page1 = (_scrollView.contentOffset.x / CGRectGetWidth(_scrollView.frame));
