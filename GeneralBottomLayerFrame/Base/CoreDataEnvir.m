...
//#define DB_LOG NSLog
#define DB_LOG(...) if(0)NSLog(__VA_ARGS__)
...
static char kCoreDataEnvirementKey;

@implementation NSManagedObjectContext(CoreDataEnvirement)

@dynamic envirement;

-(void)setEnvirement:(CoreDataEnvir *)envirement{
    objc_setAssociatedObject(self, &kCoreDataEnvirementKey, envirement, OBJC_ASSOCIATION_ASSIGN);
}

-(CoreDataEnvir *)envirement{
    return objc_getAssociatedObject(self,&kCoreDataEnvirementKey);
}
...
+ (void)_renameDatabaseFile
{
    ...
    for (NSString *name in contents) {
        ...
        if ([fm fileExistsAtPath:checkName isDirectory:&isDir] && !isDir, [[name pathExtension] isEqualToString:@"sqlite"]) {
            ...
            DB_LOG(@"Rename sqlite database from %@ to %@ finished!", name, [[self mainInstance] databaseFileName]);
            break;
        }
    }
    DB_LOG(@"No sqlite database be renamed!");
}
...
+ (CoreDataEnvir *) instance
{
    ...
    CoreDataEnvir *a_new_db = nil;
        if ([[NSThread currentThread] isMainThread]) {
            ...
            DB_LOG(@"CoreDataEnvir on main thread!");
#endif
            a_new_db = [self mainInstance];
        }else {
#if DEBUG && CORE_DATA_ENVIR_SHOW_LOG
            DB_LOG(@"CoreDataEnvir on other thread!");
#endif
...
- (void) _initCoreDataEnvirWithPath:(NSString *)path andFileName:(NSString *) dbName
{
    DB_LOG(@"%s   %@  /  %@", __FUNCTION__,path, dbName);
    ...
    if (self.storeCoordinator == nil) {
        ...
        if (!model) {
            DB_LOG(@"You create instances' number more than 123.");
        ...
        if (![self.storeCoordinator addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:fileUrl options:options error:&error]) {
            DB_LOG(@"%s Failed! %@", __FUNCTION__, error);
            if (_rescureDelegate &&
...
- (NSArray *) fetchItemsByEntityDescriptionName:(NSString *)entityName
{
    ...
    if (error) {
        DB_LOG(@"%s, error:%@, entityName:%@", __FUNCTION__, error, entityName);
    }
    [req release];
...
- (NSArray *) fetchItemsByEntityDescriptionName:(NSString *)entityName usingPredicate:(NSPredicate *)predicate
{
    ...
    items = [self.context executeFetchRequest:req error:&error];
    if (error) {
        DB_LOG(@"%s, error:%@, entityName:%@", __FUNCTION__, [error localizedDescription], entityName);
    }
    [req release];
...
- (NSArray *) fetchItemsByEntityDescriptionName:(NSString *)entityName usingPredicate:(NSPredicate *)predicate usingSortDescriptions:(NSArray *)sortDescriptions
{
    ...
    items = [self.context executeFetchRequest:req error:&error];
    if (error) {
        DB_LOG(@"%s, error:%@", __FUNCTION__, [error localizedDescription]);
    }
    [req release];
...
- (id)dataItemWithID:(NSManagedObjectID *)objectId
{
    if (objectId && self.context) {
        ...
        @catch (NSException *exception) {
            DB_LOG(@"exce :%@", [exception description]);
            item = nil;
        }
        @finally {
...
#if DEBUG && CORE_DATA_ENVIR_SHOW_LOG
    DB_LOG(@"%s  objectID :%@; getObject :%@;", __FUNCTION__, aItem.objectID, getObject);
#endif
...
- (BOOL)saveDataBase
{
    ...
    if (!bResult) {
        if (error != nil) {
            DB_LOG(@"%s, error:%@", __FUNCTION__, error);
        }
        //Do we need rollback?
        //[context rollback];
    }
    [self.storeCoordinator unlock];
    DB_LOG(@"%s", __FUNCTION__);
	return bResult;
}
...
- (void)dealloc {
#if DEBUG && CORE_DATA_ENVIR_SHOW_LOG
    DB_LOG(@"%@", [self currentDispatchQueueLabel]);
#endif
    _create_counter --;
...
- (void)updateContext:(NSNotification *)notification
{
    ...
    if (!changedContext.envirement) {
        return;
    }
#if DEBUG && CORE_DATA_ENVIR_SHOW_LOG
    DB_LOG(@"%s %@ ->>> %@", __FUNCTION__, notification.object, self.context);
#endif
    
    NSManagedObjectContext *changeContext = notification.object;
    if (!changeContext.envirement) {
        return;
    }
    ...
    @catch (NSException *exception) {
        DB_LOG(@"exce :%@", exception);
    }
    @finally {
        //DB_LOG(@"Merge finished!");
    }
    [self.storeCoordinator unlock];
}
...
- (void)mergeChanges:(NSNotification *)notification {
#if DEBUG && CORE_DATA_ENVIR_SHOW_LOG
    DB_LOG(@"%s [%@/%@] %@", __FUNCTION__, self.context, notification.object, [self currentDispatchQueueLabel]);
#endif
...
+ (id)insertItem
{
    if (![NSThread isMainThread]) {
#if DEBUG
        DB_LOG(@"Insert item record failed, please run on main thread!");
#endif
...
+ (NSArray *)items
{
    if (![NSThread isMainThread]) {
#if DEBUG
        DB_LOG(@"Fetch all items record failed, please run on main thread!");
#endif
...
+ (NSArray *)itemsWithPredicate:(NSPredicate *)predicate
{
    if (![NSThread isMainThread]) {
#if DEBUG
        DB_LOG(@"Fetch item record failed, please run on main thread!");
#endif
...
+ (id)lastItem
{
    if (![NSThread isMainThread]) {
#if DEBUG
        DB_LOG(@"Fetch last item record failed, please run on main thread!");
#endif
...
{
#if DEBUG
    DB_LOG(@"%s thread :%u, %@", __func__, [NSThread isMainThread], [NSString stringWithCString:dispatch_queue_get_label(dispatch_get_current_queue()) encoding:NSUTF8StringEncoding]);
#endif
    id item = nil;
...
- (void)remove
{
    if (![NSThread isMainThread]) {
#if DEBUG
        DB_LOG(@"Remove data failed, cannot run on non-main thread!");
#endif
...
- (BOOL)save
{
    if (![NSThread isMainThread]) {
#if DEBUG
        DB_LOG(@"Save data failed, cannot run on non-main thread!");
#endif
