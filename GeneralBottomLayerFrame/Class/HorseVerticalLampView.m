...
@implementation HorseVerticalLampView
{
    CGRect          _topRect;
    CGRect          _middleRect;
    CGRect          _btmRect;
    ...
    CycleVerticalViewScrollDirection  _direction;
            ...
            textAlignment:(NSTextAlignment)textAlignment
{
    ...
    _tmpAnimationView2.frame = _direction == CycleVerticalViewScrollDirectionDown ? _topRect : _btmRect;
}

- (void)setDirection:(CycleVerticalViewScrollDirection)direction
{
    _direction = direction;
    _tmpAnimationView2.frame = _direction == CycleVerticalViewScrollDirectionDown ? _topRect : _btmRect;
}
...
- (void)executeAnimation
{
    ...
    [UIView animateWithDuration:_animationTime delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        _tmpMiddleView.frame = _direction == CycleVerticalViewScrollDirectionDown ? _btmRect : _topRect;
        if (_direction == CycleVerticalViewScrollDirectionDown) {
...
- (void)finished
{
    _tmpMiddleView.frame = _direction == CycleVerticalViewScrollDirectionDown ? _topRect : _btmRect;
    _indexNow++;
}
...
- (void)btnClick
{
    if(_delegate && [_delegate respondsToSelector:@selector(cycleVerticalView:didClickItemIndex:)]){
        [_delegate cycleVerticalView:self didClickItemIndex:_indexNow%(_dataSource.count)];
    }
}
