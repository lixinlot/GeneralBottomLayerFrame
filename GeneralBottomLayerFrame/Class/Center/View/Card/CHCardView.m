...
- (void)tapGestHandle:(UITapGestureRecognizer *)tapGest {
    if ([self.delegate respondsToSelector:@selector(cardView:didClickItemAtIndex:)]) {
        [self.delegate cardView:self didClickItemAtIndex:tapGest.view.tag];
    }
}

- (void)cardItemViewDidRemoveFromSuperView:(CHCardItemView *)cardItemView withLeft:(BOOL)isLift{
    if ([self.delegate respondsToSelector:@selector(cardView:didRemoveItemAtIndex:withLeft:)]) {
        [self.delegate cardView:self didRemoveItemAtIndex:_itemViewCount withLeft:isLift];
    }
    if (_itemViewCount > 0) {
        _itemViewCount -= 1;
        if (_itemViewCount == 0) {
