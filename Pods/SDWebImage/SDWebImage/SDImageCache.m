...
- (nonnull instancetype)initWithNamespace:(nonnull NSString *)ns
    ...
    if ((self = [super init])) {
        ...
        // Init the memory cache// NSCache对象，用于内存缓存
        _memCache = [[SDMemoryCache alloc] init];
        _memCache.name = fullNamespace;

        // Init the disk cache// 创建文件缓存目录路径
        if (directory != nil) {
            ...
            @autoreleasepool {
                ...
                if (!data && image) {
                    ...
                    if (SDCGImageRefContainsAlpha(image.CGImage)) {
                        // 包含透明度信息，属于PNG格式的图片，image转成NSData
                        format = SDImageFormatPNG;
                    } else {
                        // 不包含透明度信息，属于JPEG格式的图片，image转成NSData
...
- (void)backgroundDeleteOldFiles {
    ...
    //开启一个用于后台执行任务的task，并且设置超时的block
    //该方法允许在程序进入后台的时候提供一段时间用于执行app未完成的任务，如果超出这段时间，则执行ExpirationHandler的block，调用endBackgroundTask:方法终止task。
    ...
    //cleanDiskWithCompletionBlock方法是在后台执行的代码，执行完成后会调用endBackgroundTask:方法终止task。
...
- (NSUInteger)getSize {
    ...
    dispatch_sync(self.ioQueue, ^{
        //返回以提供的路径为根的NSDirectoryEnumerator。如果无法创建枚举器，则返回NULL。 因为NSDirectoryEnumerator是NSEnumerator的子类，所以返回的对象可以在for ... in构造中使用
        NSDirectoryEnumerator *fileEnumerator = [self.fileManager enumeratorAtPath:self.diskCachePath];
        for (NSString *fileName in fileEnumerator) {
            NSString *filePath = [self.diskCachePath stringByAppendingPathComponent:fileName];
            //返回键/值对的NSDictionary<NSFileAttributeKey, id>，其中包含相关路径中项目（文件，目录，符号链接等）的属性。 如果此方法返回'nil'，则会在'error'参数中通过引用返回NSError。 此方法不遍历终端符号链接。此方法替换fileAttributesAtPath：traverseLink：。
